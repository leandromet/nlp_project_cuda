...
# Define landmarks for bacurizinho area
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('bacurizinho_analysis.log'),
...
def extract_bacurizinho(vrt_path, output_dir):
    """Extract and analyze land cover changes for Terra Indigena bacurizinho area."""
    # Terra Indigena bacurizinho bounding box
    lat_range = (-6.5, -5.5)  # South to North
    lon_range = (-46.2, -45.2)   # West to East
    ...
    try:
        ...
        logging.error(f"Error processing Terra Indigena bacurizinho: {str(e)}", exc_info=True)
        raise
...
def visualize_results(output_dir):
    try:
        ...
        ax.set_title('Terra Indigena bacurizinho Land Cover 2023 (EPSG:4326)')
        
        # Add key landmarks for reference
        landmarks = {
            "Bacurizinho Indigenous Land": (-6.0, -46.0),
            "Porquinhos Indigenous Land": (-5.8, -45.5),
            "Grajaú City": (-5.8136, -46.1464),
            "Barra do Corda City": (-5.5045, -45.2485),
            "Indigenous Cultural Center": (-5.9, -45.7)
        }
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(output_dir, 'bacurizinho_2023.png'),
            dpi=300,
            bbox_inches='tight'
        )
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(output_dir, 'bacurizinho_changes.png'),
            dpi=300,
            bbox_inches='tight'
        )
        plt.close()
...
if __name__ == '__main__':
    ...
    OUTPUT_DIR = 'bacurizinho_results'
    
    try:
        logging.info("Starting Terra Indigena bacurizinho analysis")
        zarr_path = extract_bacurizinho(VRT_FILE, OUTPUT_DIR)
