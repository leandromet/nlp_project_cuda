...
# Define landmarks for iguacu area
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('iguacu_analysis.log'),
...
def extract_iguacu(vrt_path, output_dir):
    """Extract and analyze land cover changes for parque iguacu area."""
    # parque iguacu bounding box
    lat_range = (-25.7, -24.7)  # South to North
    lon_range = (-54.7, -53.7)   # West to East
    ...
    try:
        ...
        logging.error(f"Error processing parque iguacu: {str(e)}", exc_info=True)
        raise
...
def visualize_results(output_dir):
    try:
        ...
        ax.set_title('parque iguacu Land Cover 2023 (EPSG:4326)')
        
        # Add key landmarks for reference
        landmarks = {
            "Iguaçu National Park": (-25.5, -54.0),
            "Foz do Iguaçu City": (-25.516, -54.588),
            "Cataratas do Iguaçu": (-25.695, -54.436),
            "Puerto Iguazú City": (-25.599, -54.573),
            "Ciudad del Este": (-25.509, -54.611)
        }
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(output_dir, 'iguacu_2023.png'),
            dpi=300,
            bbox_inches='tight'
        )
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(output_dir, 'iguacu_changes.png'),
            dpi=300,
            bbox_inches='tight'
        )
        plt.close()
...
if __name__ == '__main__':
    ...
    try:
        logging.info("Starting parque iguacu analysis")
        zarr_path = extract_iguacu(VRT_FILE, OUTPUT_DIR)
