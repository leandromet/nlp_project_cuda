...
def store_changes(root, full_changes, VRT_BLOCK_SIZE):
    root.zeros(
        'changes',
...
def store_transitions(root, full_transitions):
    root.zeros(
        'transitions',
...
def store_data(root, src, full_window):
    data = src.read(window=full_window)
...
def store_first_last_year(root, src, full_window, FILL_VALUE, VRT_BLOCK_SIZE):
    try:
...
def extract_grid_data_optimized(root, vrt_path, polygon_coords, output_dir, full_changes,full_transitions ):
        ...
        with rasterio.Env(**rasterio_config), rasterio.open(vrt_path) as src:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with ProcessPoolExecutor(max_workers=4) as executor:
                    futures = [
                        executor.submit(store_changes, root, full_changes, VRT_BLOCK_SIZE),
                        executor.submit(store_transitions, root, full_transitions),
                        executor.submit(store_data, root, src, full_window),
                        executor.submit(store_first_last_year, root, src, full_window, FILL_VALUE, VRT_BLOCK_SIZE)
                    ]
                ...
                with ProcessPoolExecutor(max_workers=4) as executor:
                    futures = [
                        executor.submit(store_changes),
                        executor.submit(store_transitions),
                        executor.submit(store_data),
                        executor.submit(store_first_last_year)
                    ]
                    for future in as_completed(futures):
                        try:
                            future.result()
                ...
                # Initialize failed_tiles as an empty list if not already defined
                failed_tiles = []
...
def create_sankey_diagrams(root, output_dir):
    ...
    for start_year, end_year in decades:
        ...
        # Filter out transitions where no change occurred (from_cls == to_cls)
        filtered_trans = np.copy(decade_trans)
        for idx in range(len(present_classes)):
            filtered_trans[idx, idx] = 0  # Remove self-to-self transitions
        
        # Create diagram
        title = f"{grid_name} Land Cover Changes {start_year}-{end_year} (Excluding No Change)"
...
if __name__ == '__main__':
    ...
    try:
        ...
        # Initialize required arguments for extract_grid_data_optimized
        root = zarr.open('output.zarr', mode='w')  # Example Zarr root
        full_changes = np.zeros((512, 512), dtype='uint8')  # Example placeholder
        full_transitions = np.zeros((256, 256), dtype='uint64')  # Example placeholder

        zarr_path, grid_output_dir = extract_grid_data_optimized(
            root, VRT_FILE, POLYGON_8x8, OUTPUT_BASE_DIR, full_changes, full_transitions
        )
