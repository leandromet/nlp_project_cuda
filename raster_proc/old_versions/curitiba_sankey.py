...
# Define landmarks for Curitiba area
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('curitiba_analysis.log'),
...
def extract_curitiba(vrt_path, output_dir):
    """Extract and analyze land cover changes for cidade de  curitiba area."""
    # cidade de  curitiba bounding box
    lat_range = (-26, -25)  # South to North
    lon_range = (-49.7, -48.7)   # West to East
    ...
    try:
        ...
        logging.error(f"Error processing cidade de  curitiba: {str(e)}", exc_info=True)
        raise
...
def visualize_results(output_dir):
    try:
        ...
        ax.set_title('cidade de  curitiba Land Cover 2023 (EPSG:4326)')
        
        # Add key landmarks for reference
        landmarks = {
            "Botanical Garden of Curitiba": (-25.4421, -49.2407),
            "Oscar Niemeyer Museum": (-25.4094, -49.2654),
            "Tanguá Park": (-25.3875, -49.2712),
            "Wire Opera House": (-25.3882, -49.2718),
            "Barigui Park": (-25.4260, -49.3083)
        }
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(output_dir, 'curitiba_2023.png'),
            dpi=300,
            bbox_inches='tight'
        )
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(output_dir, 'curitiba_changes.png'),
            dpi=300,
            bbox_inches='tight'
        )
        plt.close()
...
if __name__ == '__main__':
    ...
    OUTPUT_DIR = 'curitiba_results'
    
    try:
        logging.info("Starting cidade de  curitiba analysis")
        zarr_path = extract_curitiba(VRT_FILE, OUTPUT_DIR)
