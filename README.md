...
```
executed code with simplest model:
...
Finally a script with more data and log, minor answer so as not to look too incorrect and know GPU is used
```python
...
Changed prompt for input_text = "Are forests in danger from global warming? And are they responsible for it?"
